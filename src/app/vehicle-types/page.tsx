...
import { Plus, Search, RefreshCw, Car, Edit, Loader2 } from "lucide-react";
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 bg-slate-900/60 border border-slate-800/80 rounded-2xl backdrop-blur-xl">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => handleOpenModal()} className="px-4 py-2 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white text-xs font-semibold shadow-lg transition-colors flex items-center gap-2">
      ...
      <div className="bg-slate-900/60 border border-slate-800/80 rounded-2xl p-6 backdrop-blur-xl space-y-4">
        <div className="relative max-w-md">
          ...
          <input
            ...
            className="w-full bg-slate-950/50 border border-slate-800 rounded-xl pl-10 pr-4 py-2 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500"
          />
        </div>
              ...
              {loading ? (
                <tr>
                  <td colSpan={4} className="p-8 text-center text-slate-500">
                    <Loader2 className="w-6 h-6 animate-spin mx-auto text-cyan-500 mb-2" /> Loading...
                  </td>
                </tr>
              ) : items.length === 0 ? (
                <TableEmptyState
                  ...
                  description="Belum ada jenis/kategori kendaraan terdaftar."
                  searchTerm={search}
                ...
                items.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-800/30 font-medium">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md p-6 space-y-4 shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div>
                ...
                <input type="text" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-cyan-500" />
              </div>
              <div className="flex justify-end gap-2 pt-4">
                ...
                <button type="submit" disabled={submitting} className="px-4 py-2 rounded-xl bg-cyan-600 text-white text-xs font-semibold disabled:opacity-50">{submitting ? "Saving..." : "Save"}</button>
              </div>
            </form>
          </div>
        </div>
