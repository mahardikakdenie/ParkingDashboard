...
export function Sidebar() {
  ...
  useEffect(() => {
    ...
    async function fetchMenuData() {
      try {
        ...
        const rawMenus: MenuResponse[] =
          menusRes.status === "fulfilled" && Array.isArray(menusRes.value)
            ? menusRes.value
            : [];
        ...
        topLevelMenus.forEach((item) => {
          ...
          const children = (rawChildren as any[]).map((c) =>
            typeof c === "string"
              ? { label: c, href: "#" }
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside
        className={`w-64 bg-slate-900/95 backdrop-blur-2xl border-r border-slate-800/80 flex flex-col h-full shrink-0 text-slate-300
          fixed md:static inset-y-0 left-0 z-50 transform md:transform-none transition-transform duration-300 ease-in-out shadow-2xl shadow-indigo-950/30
          ${sidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}
        `}
      >
        {/* Header with Logo */}
        <div className="p-4 sm:p-5 border-b border-slate-800/80 bg-linear-to-b from-slate-900/95 to-slate-900/70 relative z-20">
          <div className="flex items-center justify-between">
            <CompanyLogo size="md" showStatusDot={true} variant="full" />
            <button
              onClick={() => setSidebarOpen(false)}
